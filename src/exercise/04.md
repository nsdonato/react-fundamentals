...
El problema es que si usamos el nombre de la función en minuscula, cuando Babel
lo transpila, este pasa a ser un string, no nos olvidemos que transpilaría a
algo así: React.CreateElement('message', ...etc) Es por eso que mismo React tira
en la consola que si estamos tratando de crear un componente, lo iniciemos con
la primer letra en mayuscula.

```jsx
// De esta manera podemos hacer algo así como un strategy de componentes
...
Recomendaciones: In general, we don’t recommend not passing a value for a prop,
because it can be confused with the ES6 object shorthand {foo} which is short
for {foo: foo} rather than {foo: true}. This behavior is just there so that it
