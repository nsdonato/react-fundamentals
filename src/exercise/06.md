...
- A través de su índice: event.target.elements[0].value
- A través de los elementos objeto por su atributo nameo id:
  event.target.elements.usernameInput.value
- PreventDefault justamente sirve para prevenir el comportamiento por default
  del navegador al submitir, ya que de esta manera se estaría haciendo un
  request y refrescaria la pagina, haciendo que la dirección cambie a la misma
  pagina juinto a los parametros del fomrulario por queryString, salvo que los
  inputs no tengan el atributo name
- Syntathic event, no es un evento real, es un objeto que crea react, que simula
  ser el evento real del browser, es por cuestiones de performance. Si queremos
  obtener el evento real, podemos hacer event.nativeEvent para imprimir por
- ejemplo el nodo de form en si mismo, podemos utilizar
  console.dir(event.target) en caso de que se haya hecho el submit
- los screen readers necesitan que el label este asociado al input, por ende se
  debe usar htmlFor y el nombre del id o name?
- Una referencia es un objeto que se mantiene consistente entre renderizados.
  Tiene una propiedad current, la que podemos actualizar en cualquier momento.
  En el caso de interactuar con los nodes del DOM, podes pasar una referncia una
  referencia a un elemento de react y react va a setear el valor actual
  (current) en el DOM donde el nodo es renderizado. So if you create an inputRef
  object via React.useRef, you could access the value via:
  inputRef.current.value https://reactjs.org/docs/hooks-reference.html#useref
- Cuándo usar las referencias: Hay algunos buenos casos de uso para las
  referencias:
  ...
  - Evite el uso de referencias para cualquier cosa que se pueda hacer de forma
    declarativa.

  Por ejemplo, en lugar de exponer los métodos open()y close()en un
  Dialogcomponente, pase un isOpenaccesorio.
...
- Para los screen readers es importante que el elemento que muestre el alerta,
  tenga el atributo y valor: role="alert"
- Si no pasas un onChange handler a un input, eso va a significar que el input
  es readonly, por ende vas a escribir y no va a funcionar...
- en react se puede usar el atributo defaultValue={'algo por defecto'}
