...
- Si vuelve a renderizar esa lista con un elemento agregado, React no sabe
  realmente si agregó un elemento en el medio, al principio o al final. Y lo
  mismo ocurre cuando elimina un elemento (no sabe si eso sucedió en el medio,
  al principio o al final).
- La propiedad key, es un atributoe special de react que lo ayuda a hacer un
  aseguimiento de los elementos entre actualizaciones.
- Si no lo pones al renderizar una lista, React puede confundir las cosas. La
  solución es darle ese atributo a cada elemento.
- Profundicemos un poco más:

Si vuelve a renderizar esa lista con un elemento agregado, React no sabe
realmente si agregó un elemento en el medio, al principio o al final. Y lo mismo
ocurre cuando elimina un elemento (no sabe si eso sucedió en el medio, al
principio o al final).

Como desarrollador que escribió el código, lo sabemos, pero React no e intenta
comparar el antes y después y no sabe que elemento se tomo de una posición a
otra.

A veces no es gran cosa, porque la mejor suposición de React es correcta y
funciona bien. Sin embargo, si alguno de esos elementos de React representa un
componente que mantiene el estado (como el valor de un estado de entrada o de
enfoque), eso puede ser bastante problemático, como lo demuestra este ejercicio.

Para resolver este problema, necesitamos darle a React una pista para que pueda
asociar los elementos antiguos de React con los nuevos que le estamos dando
debido al cambio. Hacemos esto usando el atributo key.

https://kentcdodds.com/blog/understanding-reacts-key-prop
https://epicreact.dev/why-react-needs-a-key-prop/

No recomendamos usar índices para keys si el orden de los ítems puede cambiar.
Esto puede impactar negativamente el rendimiento y puede causar problemas con el
estado del componente Para ayudarlo a decidir, reuní tres condiciones que estos
ejemplos tienen en común: la lista y los elementos son estáticos, no se calculan
y no cambian; los elementos de la lista no tienen ID; la lista nunca se reordena
ni se filtra.
