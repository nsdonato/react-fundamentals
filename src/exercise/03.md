...
Babel is written in JavaScript we can actually run it in the browser to compile
our code on the fly and that’s what we’ll do in this exercise.
...
Si usas un script y le pones // to `type="text/babel"` so babel will compile
this code for the browser to run.

si usas "module" lo toma como cualquier modulo js

Links: https://reactjs.org/docs/dom-elements.html#differences-in-attributes

JSX: It is called JSX, and it is a syntax extension to JavaScript.

Las expresiones de JS van dentro de {}. Recordar que una expresión es algo que
le preguntamos a js y "nos responde" ej: {2 + 2} JS responde 4 Interpolation:
`${greeting} ${subject}`

El extra credit, en realidad había que hacerlo así:
